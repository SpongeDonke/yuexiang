    ...
    <div class="profile-card">
      <div class="profile-header">
        <div class="avatar-section">
          <div class="avatar-ring admin-ring">
            <i class="fas fa-shield-alt avatar-icon"></i>
          </div>
        ...
        <h1 class="profile-title">管理员中心</h1>
        <p class="welcome-text">系统管理权限控制台</p>
      </div>
      ...
      <form @submit.prevent="updateProfile" class="profile-form">
        ...
        <div class="action-area">
          <button type="submit" class="btn-save admin-btn">
      ...
      <transition name="message-fade">
        <div v-if="message" class="success-message admin-msg">
    ...
    <div class="info-cards">
      <div class="info-card system-info">
        ...
        <div class="card-content">
          <h4>系统状态</h4>
          <p>监控系统运行状况，确保平台稳定运行。</p>
        </div>
      </div>
      <div class="info-card security-admin">
        <i class="fas fa-user-secret"></i>
        <div class="card-content">
          <h4>安全审计</h4>
          <p>定期检查用户数据，保障信息安全合规。</p>
        </div>
      </div>
    </div>
  </div>
...
.profile-card {
  ...
  border-radius: 20px;
  padding: 40px;
  box-shadow: 0 15px 50px rgba(255, 77, 79, 0.25);
...
@keyframes glow {
  0%, 100% { box-shadow: 0 8px 32px rgba(255, 214, 102, 0.3); }
  50% { box-shadow: 0 8px 48px rgba(255, 214, 102, 0.6); }
}

.avatar-icon { font-size: 48px; color: white; }
...
.input-group label i { color: #ff4d4f; font-size: 16px; }
...
.input-wrapper.disabled { background: #f5f5f5; opacity: 0.75; }
...
.phone-prefix { background: #fff1f0; color: #ff4d4f; font-weight: 600; }
...
.admin-btn:hover {
  ...
  box-shadow: 0 6px 22px rgba(114, 46, 209, 0.45);
}
...
.system-info > i { color: #722ed1; background: #f9f0ff; }
.security-admin > i { color: #ff4d4f; background: #fff1f0; }
