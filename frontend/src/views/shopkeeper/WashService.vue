    ...
    <div class="header-section">
      <div class="header-left">
        ...
        <div>
          ...
          <p class="subtitle">管理您店铺提供的洗护服务项目</p>
        </div>
      </div>
    ...
    <div v-else class="content-area">
      <div class="stats-bar">
        ...
        <div class="stat-item">
          ...
          <span>平均价格</span>
          <strong>¥{{ services.length ? (services.reduce((sum, s) => sum + (s.price || 0), 0) / services.length).toFixed(2) : '0.00' }}</strong>
        </div>
      </div>
        ...
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr v-if="services.length === 0" class="empty-row">
              <td colspan="6"><i class="fas fa-inbox"></i> 暂无服务数据</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div v-if="showEditDialog" class="modal-overlay" @click.self="showEditDialog = false">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <button class="btn-save" @click="saveService"><i class="fas fa-save"></i> 保存修改</button>
        </div>
      </div>
    </div>
...
const addForm = ref({ name: '', typeId: null, price: null, description: '' })
const editForm = ref({})
const currentDeleteId = ref(null)

onMounted(async () => {
  await Promise.all([loadShop(), loadTypes()])
})
...
const loadServices = async () => {
  ...
  try {
    const response = await axios.get(`/api/wash-services/shop/${shopId.value}`)
...
const addService = async () => {
  try {
    const serviceData = { ...addForm.value, shopId: shopId.value }
    await axios.post('/api/wash-services', serviceData)
    await loadServices()
    showAddDialog.value = false
    addForm.value = { name: '', typeId: null, price: null, description: '' }
  } catch (error) {
...
const editService = (service) => {
  editForm.value = { ...service, typeId: service.type?.id }
  showEditDialog.value = true
}

const saveService = async () => {
  try {
    const serviceData = { ...editForm.value, shopId: shopId.value }
    await axios.post('/api/wash-services', serviceData)
    await loadServices()
